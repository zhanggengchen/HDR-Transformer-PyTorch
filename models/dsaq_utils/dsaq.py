...
class _ActQ(nn.Module):
    def __init__(self, **kwargs_q):
        ...
        if self.nbits < 0:
            ...
            return
        self.alpha = Parameter(torch.Tensor(1))
        self.zero = Parameter(torch.Tensor(1))
        self.register_buffer('init_state', torch.zeros(1))
...
class ActOurs(_ActQ):
    ...
    def forward(self, x):
        ...
        if self.training and self.init_state == 0:
            ...
            self.zero.data.copy_(self.alpha * Qn)
        ...
        x = round_pass(((x - self.zero) / alpha).clamp(Qn, Qp)) * alpha + self.zero
...
class Conv2dOurs(_Conv2dQ):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, nbits_w=4, nbits_a=4, quant_act=True, **kwargs):
